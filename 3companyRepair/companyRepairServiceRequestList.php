...
$stmt = $conn->prepare("SELECT sl.*, e.EquipmentName, u.Name as CompanyName 
                        FROM servicelog sl 
                        JOIN equipment e ON sl.EquipmentID = e.EquipmentID
                        JOIN users u ON sl.CompanyID = u.UserID
                        WHERE sl.Status = 'Approved' AND sl.CompanyID = ?");
      ...
      <tbody>
        ...
        if ($result->num_rows > 0) {
          while ($row = $result->fetch_assoc()) {
            ...
            echo "<td><a href='companyRepairServiceStatus.php?serviceID=" . $row['ServiceID'] . "'><button class='buttonStatus'>Check Status</button></a></td>";
            echo "</tr>";
          }
        } else {
