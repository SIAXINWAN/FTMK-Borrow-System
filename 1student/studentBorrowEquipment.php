...
$stmt3 = $conn->prepare("SELECT UserID as LecturerId,Name FROM dummy WHERE Role =? ");
    ...
    <form action="../Borrow/submitBorrowApplication.php?id=<?php echo htmlspecialchars($equipmentID); ?>" method="post">
      ...
      <div class="form-section">
        <div class="form-group">
          ...
          <select name="lecturerId" id="lecturerId" required>
            <option value="" selected disabled>Please choose your lecturer</option>
            <?php
            if ($lresult->num_rows > 0) {
              while ($row = $lresult->fetch_assoc()) {
                echo "<option value='" . htmlspecialchars($row["LecturerId"]) . "'>" . htmlspecialchars($row["Name"]) . "</option>";
              }
            }
            ?>
          </select>
